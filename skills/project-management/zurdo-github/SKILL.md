...
| `sync-status` | Reads `.zurdo/<slug>/prd.json` (`--slug`, else newest) and applies the status mapping: close, label swap, comment. Refreshes the Status column of the epic's task table in place. Closes, labels, and the refresh are idempotent; the comment is appended on every run, so sync once per settled run. |
